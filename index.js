...
const finalHome = fifaData.filter (item => 
...
function getWinners(data, callback) {
    const winners = callback(data).map(function(item){
...
function getWinnersByYear(cbgetWinners, cbgetYears) {
    ...
    for(let i = 0; i < cbgetWinners.length; i++){
        arr.push(` In ${cbgetYears[i]}, ${cbgetWinners[i]} won the world cup!`)
    }
    return arr
};
...
function getAverageGoals(data) {
    let homeAvg = data.reduce ((acc, cur) => ({'Home Team Goals': acc['Home Team Goals'] + cur['Home Team Goals']/data.length}));

    let awayAvg = data.reduce ((acc, cur) => ({'Away Team Goals': acc['Away Team Goals'] + cur['Away Team Goals']/data.length}));
